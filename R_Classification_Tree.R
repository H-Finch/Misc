...
chaid.tree <-chaid(Target~Gender+AGE_BKT,data=CTDF, control = ctrl)
print(chaid.tree)
plot(chaid.tree)
...
library(ineq)
gini = ineq(hrdata.dev$predict.score[,2], type="Gini")

with(hrdata.dev, table(Attrition, predict.class))
auc
KS
gini

## Syntax to get the node path
tree.path <- path.rpart(ptree, node = c(1, 15))
